...
TEST(Foo, Foo) { LOG(INFO) << "Some test!"; }
...
TEST(Foo, ServiceUsersTest) {
  ...
  for (auto&& user : users) service.createUser(user, pswdHash);
  ...
  ASSERT_THROW(service.removeUser(users.at(0), pswdHash + "0"),
               std::runtime_error);
...
TEST(Foo, NotesTest) {
  ...
  ASSERT_THROW(service.removeNote(users.back(), 2), std::invalid_argument);
  service.removeNote(users.back(), 1);
  service.removeNote(users.back(), 0);
  ASSERT_THROW(service.removeNote(users.back(), 0), std::invalid_argument);
